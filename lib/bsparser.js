    ...
    function Parser(text) {
        ...
        function parseSimpleCommand() {
            ...
            if (token.type == TokenType.Name) {
                ...
                if (name == 'break')
                    return new bscommands.BreakCommand();
                    
                if (name == 'continue')
                    return new bscommands.ContinueCommand();
                ...
                if (name == 'if') {
                    ...
                    for (var token = lexer.nextToken(); !isEnd(token); token = lexer.nextToken()) {
                        ...
                        if (iselse)
                            elsecommands.push(self.parseCommand());                        
                        else
                            thencommands.push(self.parseCommand());                        
                    }
                ...
                if (name == 'for') {
                    ...
                    var fromexpression = self.parseExpression();
                    parseToken('to', TokenType.Name);
                    var toexpression = self.parseExpression();
                    var stepexpression;
                    
                    if (tryParseToken('step', TokenType.Name))
                        stepexpression = self.parseExpression();
